...
//1. new&pseudo
//QString MaClasse::user_to_string(QString pseudo)
//{
//  Qstring result=QString("new&")+QString("&")+pseudo;
//  return result;
//}
...
QString Baptiste::baptiste_listedecartes (std::vector<Carte*> paquet, unsigned int index){
  QString result=QString("paquet&")+QString::number(paquet.size()) + "&" +QString::number(index);
...
QString Baptiste::baptiste_fullpaquet (std::vector<std::vector<Carte*>> cp){
  int n = 0;
  for (int i = 0; i < cp.size(); i++) {
      n += cp[i].size();
  }
  QString result=QString("paquet&")+QString::number(n);
  for(unsigned int i=0;i<cp.size();i++)
  {
      for (unsigned int j = 0; j < cp[i].size(); j++) {
          result += QString("&")+QString::number(i)+"#"+QString::number(cp[i][j]->idCarte)+QString("#")+QString::number(cp[i][j]->trouver)+QString("#")+cp[i][j]->typeCarte;
      }
  }
  return result;
}
...
//QList<struct salon> MaClasse::string_to_lobby(QString buf)
//{
//  QStringList liste = buf.split('&');
//  QStringList inf_salon;
...
//    cur_salon.id=info_salon[1];
//    cur_salon.authNeeded=info_salon[2];
//    cur_salon.nb_clients=info_salon[3];
//    cur_salon.nb_clientsmax=info_salon[4];
//    cur_salon.hasStarted=info_salon[5];
...
QString Baptiste::baptiste_lobby (QList<struct salon> salons)
{
    QString result=QString("lobby&")+QString::number(salons.size());
    int i=0;
    qDebug() << salons.size() << endl;
    if (!salons.isEmpty()) {
        for(i=0;i<salons.size();i++)
        {
          qDebug() << result << endl;
          //result+=QString("&")+salons[i]->name+ "#" + QString::number(salons[i]->id)+QString("#")+QString::number(salons[i]->authNeeded)+QString("#")+QString::number(salons[i]->clients.size())+QString("#")+QString::number(salons[i]->nbrJoueursMax)+QString("#")+QString::number(salons[i]->hasStarted);
          result += "&" + salons[i].name + "#" + QString::number(salons[i].id) + "#" + QString::number(salons[i].visibility);
        }
    }
